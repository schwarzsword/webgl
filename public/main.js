    ...
    let draw = function (vertexShaderText, fragmentShaderText, boxImg, modelImage, model, torusImg, ballImg) {
        ...
        //drawing
        {
            ...
            //ashes
            {
                ...
                let ashrsInd = ashes.indices;
                let ashedTex = ashes.texCoords;
                ...
                let ashesTexCoordVertexBufferObject = gl.createBuffer();
                gl.bindBuffer(gl.ARRAY_BUFFER, ashesTexCoordVertexBufferObject);
                gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(ashedTex), gl.STATIC_DRAW);
                ...
                gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(ashrsInd), gl.STATIC_DRAW);


                gl.bindBuffer(gl.ARRAY_BUFFER, ashesVertexBufferObject);
                let ashesPositionAttribLocation = gl.getAttribLocation(program, 'vertPosition');
                gl.vertexAttribPointer(
                    ashesPositionAttribLocation, // Attribute location
                    3, // Number of elements per attribute
                    gl.FLOAT, // Type of elements
                    gl.FALSE,
                    3 * Float32Array.BYTES_PER_ELEMENT, // Size of an individual vertex
                ...
                gl.enableVertexAttribArray(ashesPositionAttribLocation);

                gl.bindBuffer(gl.ARRAY_BUFFER, ashesTexCoordVertexBufferObject);
                let ashesTexCoordAttribLocation = gl.getAttribLocation(program, 'vertTexCoord');
                gl.vertexAttribPointer(
                    ashesTexCoordAttribLocation, // Attribute location
                    2, // Number of elements per attribute
                    gl.FLOAT, // Type of elements
                    gl.FALSE,
                    2 * Float32Array.BYTES_PER_ELEMENT, // Size of an individual vertex
                ...
                gl.enableVertexAttribArray(ashesTexCoordAttribLocation);

                gl.bindBuffer(gl.ARRAY_BUFFER, ashesNormalsBufferObject);
                let ashesNormalAttribLocation = gl.getAttribLocation(program, 'vertNormal');
                gl.vertexAttribPointer(
                    ashesNormalAttribLocation,
                    3,
                    gl.FLOAT,
                    gl.TRUE,
                    3 * Float32Array.BYTES_PER_ELEMENT,
                ...
                gl.enableVertexAttribArray(ashesNormalAttribLocation);

                //
                // Create texture
                //

                let ashesTexture = gl.createTexture();
                gl.bindTexture(gl.TEXTURE_2D, ashesTexture);
                gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
                gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
                gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
                gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
                ...
                gl.bindTexture(gl.TEXTURE_2D, ashesTexture);
                gl.activeTexture(gl.TEXTURE0);
                gl.drawElements(gl.TRIANGLES, ashrsInd.length, gl.UNSIGNED_SHORT, 0);
            }
        }
